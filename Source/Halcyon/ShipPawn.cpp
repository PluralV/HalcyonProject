...
AShipPawn::AShipPawn()
{
	...
	HullMesh->BodyInstance.bSimulatePhysics = false;
...
void AShipPawn::Fire() {
	// GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("fire action called"));
	if (AShipPlayerController* ASPC = Cast<AShipPlayerController>(Controller)) {
		// GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("paused???/"));
		if (ASPC->GetPauseStatus()) return;
	}
	if (CurrentTarget) {
		// // GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("current target found"));
		if (AShipPawn* ShipTarget = Cast<AShipPawn>(CurrentTarget)) {
			// GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Yellow, TEXT("target is a ship"));
